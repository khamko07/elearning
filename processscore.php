...
foreach ($exercises as $exercise) {
    ...
    if (isset($_POST[$questionName])) {
        ...
        if ($userAnswer == $correctAnswer) {
            ...
            $lessonIdForScore = $topicid > 0 ? $topicid : $lessonid;
            $sql = "INSERT INTO tblscore (StudentID, LessonID, ExerciseID, Score, Submitted, NoItems) 
                    VALUES ('{$studentid}', '{$lessonIdForScore}', '{$exercise->ExerciseID}', 1, 1, 1)
            ...
            $lessonIdForScore = $topicid > 0 ? $topicid : $lessonid;
            $sql = "INSERT INTO tblscore (StudentID, LessonID, ExerciseID, Score, Submitted, NoItems) 
                    VALUES ('{$studentid}', '{$lessonIdForScore}', '{$exercise->ExerciseID}', 0, 1, 1)
...
$redirectId = $topicid > 0 ? "topic={$topicid}" : "id={$lessonid}";
redirect("index.php?q=quizresult&{$redirectId}&score={$score}&correct={$correctAnswers}&total={$totalQuestions}");
